...
$(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/cli/$(SRC_DIR)/%.l.cpp : $(PKG_DIR)/cli/$(SRC_DIR)/%.l
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    LEX $(notdir $@) $(PRT_MODE)"
	$(AT)$(LEX) $(if $(PKG_cli_MODE_$(MODE)_L_FLAGS),$(PKG_cli_MODE_$(MODE)_L_FLAGS),$(if $(PKG_cli_L_FLAGS),$(PKG_cli_L_FLAGS),$(if $(MODE_$(MODE)_L_FLAGS),$(MODE_$(MODE)_L_FLAGS),$(CMN_L_FLAGS)))) -o$@ -P$(patsubst %.l.cpp,%,$(notdir $@)) $<

$(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/cli/$(SRC_DIR)/%.y.cpp : $(PKG_DIR)/cli/$(SRC_DIR)/%.y
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    YACC $(notdir $@) $(PRT_MODE)"
	$(AT)$(YACC) $(if $(PKG_cli_MODE_$(MODE)_Y_FLAGS),$(PKG_cli_MODE_$(MODE)_Y_FLAGS),$(if $(PKG_cli_Y_FLAGS),$(PKG_cli_Y_FLAGS),$(if $(MODE_$(MODE)_Y_FLAGS),$(MODE_$(MODE)_Y_FLAGS),$(CMN_Y_FLAGS)))) -p $(patsubst %.y.cpp,%,$(notdir $@)) -o $@ $<

$(TMP_TOP)/$(MODE)/$(TMP_DEP)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cpp.d : $(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cpp
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    DEP $(notdir $@) $(PRT_MODE)"
	$(AT)$(CXX) -MM $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/cli/$(INC_DIR)) $(addprefix -I,$(PKG_cli_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_cli_INCS)) $(MODE_INCS) $< > $@.$$$$; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/cli/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ > $@; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/$(PKG_DIR)/cli/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ >> $@; \
	rm $@.$$$$

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cpp.o : $(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cpp
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    CXX $(notdir $<) $(PRT_MODE)"
	$(AT)$(CXX) $(if $(PKG_cli_MODE_$(MODE)_C_FLAGS),$(PKG_cli_MODE_$(MODE)_C_FLAGS),$(if $(PKG_cli_C_FLAGS),$(PKG_cli_C_FLAGS),$(if $(MODE_$(MODE)_C_FLAGS),$(MODE_$(MODE)_C_FLAGS),$(CMN_C_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/cli/$(INC_DIR)) $(addprefix -I,$(PKG_cli_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_cli_INCS)) $(MODE_INCS) -c -o $@ $<

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cpp.o : $(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cpp
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    CXX $(notdir $<) $(PRT_MODE) ($(C_DYN_FLAGS))"
	$(AT)$(CXX) $(C_DYN_FLAGS) $(if $(PKG_cli_MODE_$(MODE)_C_FLAGS),$(PKG_cli_MODE_$(MODE)_C_FLAGS),$(if $(PKG_cli_C_FLAGS),$(PKG_cli_C_FLAGS),$(if $(MODE_$(MODE)_C_FLAGS),$(MODE_$(MODE)_C_FLAGS),$(CMN_C_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/cli/$(INC_DIR)) $(addprefix -I,$(PKG_cli_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_cli_INCS)) $(MODE_INCS) -c -o $@ $<
...
$(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/interface/$(SRC_DIR)/%.l.cpp : $(PKG_DIR)/interface/$(SRC_DIR)/%.l
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    LEX $(notdir $@) $(PRT_MODE)"
	$(AT)$(LEX) $(if $(PKG_interface_MODE_$(MODE)_L_FLAGS),$(PKG_interface_MODE_$(MODE)_L_FLAGS),$(if $(PKG_interface_L_FLAGS),$(PKG_interface_L_FLAGS),$(if $(MODE_$(MODE)_L_FLAGS),$(MODE_$(MODE)_L_FLAGS),$(CMN_L_FLAGS)))) -o$@ -P$(patsubst %.l.cpp,%,$(notdir $@)) $<

$(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/interface/$(SRC_DIR)/%.y.cpp : $(PKG_DIR)/interface/$(SRC_DIR)/%.y
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    YACC $(notdir $@) $(PRT_MODE)"
	$(AT)$(YACC) $(if $(PKG_interface_MODE_$(MODE)_Y_FLAGS),$(PKG_interface_MODE_$(MODE)_Y_FLAGS),$(if $(PKG_interface_Y_FLAGS),$(PKG_interface_Y_FLAGS),$(if $(MODE_$(MODE)_Y_FLAGS),$(MODE_$(MODE)_Y_FLAGS),$(CMN_Y_FLAGS)))) -p $(patsubst %.y.cpp,%,$(notdir $@)) -o $@ $<

$(TMP_TOP)/$(MODE)/$(TMP_DEP)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cpp.d : $(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cpp
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    DEP $(notdir $@) $(PRT_MODE)"
	$(AT)$(CXX) -MM $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/interface/$(INC_DIR)) $(addprefix -I,$(PKG_interface_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_interface_INCS)) $(MODE_INCS) $< > $@.$$$$; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/interface/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ > $@; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/$(PKG_DIR)/interface/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ >> $@; \
	rm $@.$$$$

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cpp.o : $(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cpp
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    CXX $(notdir $<) $(PRT_MODE)"
	$(AT)$(CXX) $(if $(PKG_interface_MODE_$(MODE)_C_FLAGS),$(PKG_interface_MODE_$(MODE)_C_FLAGS),$(if $(PKG_interface_C_FLAGS),$(PKG_interface_C_FLAGS),$(if $(MODE_$(MODE)_C_FLAGS),$(MODE_$(MODE)_C_FLAGS),$(CMN_C_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/interface/$(INC_DIR)) $(addprefix -I,$(PKG_interface_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_interface_INCS)) $(MODE_INCS) -c -o $@ $<

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cpp.o : $(TMP_TOP)/$(MODE)/$(TMP_LNY)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cpp
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    CXX $(notdir $<) $(PRT_MODE) ($(C_DYN_FLAGS))"
	$(AT)$(CXX) $(C_DYN_FLAGS) $(if $(PKG_interface_MODE_$(MODE)_C_FLAGS),$(PKG_interface_MODE_$(MODE)_C_FLAGS),$(if $(PKG_interface_C_FLAGS),$(PKG_interface_C_FLAGS),$(if $(MODE_$(MODE)_C_FLAGS),$(MODE_$(MODE)_C_FLAGS),$(CMN_C_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/interface/$(INC_DIR)) $(addprefix -I,$(PKG_interface_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_interface_INCS)) $(MODE_INCS) -c -o $@ $<
