...
$(TMP_TOP)/$(MODE)/$(TMP_DEP)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cu.d : $(PKG_DIR)/cli/$(SRC_DIR)/%.cu
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    DEP $(notdir $@) $(PRT_MODE)"
	@$(NVCC) -M $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/cli/$(INC_DIR)) $(addprefix -I,$(PKG_cli_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_cli_INCS)) $(MODE_INCS) $< > $@.$$$$; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/cli/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ > $@; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/cli/$(PKG_DIR)/cli/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ >> $@; \
	rm $@.$$$$

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cu.o : $(PKG_DIR)/cli/$(SRC_DIR)/%.cu
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    NVCC $(notdir $<) $(PRT_MODE)"
	$(AT)$(NVCC) $(if $(PKG_cli_MODE_$(MODE)_NV_FLAGS),$(PKG_cli_MODE_$(MODE)_NV_FLAGS),$(if $(PKG_cli_NV_FLAGS),$(PKG_cli_NV_FLAGS),$(if $(MODE_$(MODE)_NV_FLAGS),$(MODE_$(MODE)_NV_FLAGS),$(CMN_NV_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/cli/$(INC_DIR)) $(addprefix -I,$(PKG_cli_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_cli_INCS)) $(MODE_INCS) -c -o $@ $<

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/$(PKG_DIR)/cli/$(SRC_DIR)/%.cu.o : $(PKG_DIR)/cli/$(SRC_DIR)/%.cu
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    NVCC $(notdir $<) $(PRT_MODE) ($(NV_DYN_FLAGS))"
	$(AT)$(NVCC) $(NV_DYN_FLAGS) $(if $(PKG_cli_MODE_$(MODE)_NV_FLAGS),$(PKG_cli_MODE_$(MODE)_NV_FLAGS),$(if $(PKG_cli_NV_FLAGS),$(PKG_cli_NV_FLAGS),$(if $(MODE_$(MODE)_NV_FLAGS),$(MODE_$(MODE)_NV_FLAGS),$(CMN_NV_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/cli/$(INC_DIR)) $(addprefix -I,$(PKG_cli_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_cli_INCS)) $(MODE_INCS) -c -o $@ $<
...
$(TMP_TOP)/$(MODE)/$(TMP_DEP)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cu.d : $(PKG_DIR)/interface/$(SRC_DIR)/%.cu
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    DEP $(notdir $@) $(PRT_MODE)"
	@$(NVCC) -M $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/interface/$(INC_DIR)) $(addprefix -I,$(PKG_interface_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_interface_INCS)) $(MODE_INCS) $< > $@.$$$$; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/interface/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ > $@; \
	sed 's,\($*\)\.o[ :]*,$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/interface/$(PKG_DIR)/interface/$(SRC_DIR)/$(notdir $<).o $@ : ,g' < $@.$$$$ >> $@; \
	rm $@.$$$$

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_STA)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cu.o : $(PKG_DIR)/interface/$(SRC_DIR)/%.cu
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    NVCC $(notdir $<) $(PRT_MODE)"
	$(AT)$(NVCC) $(if $(PKG_interface_MODE_$(MODE)_NV_FLAGS),$(PKG_interface_MODE_$(MODE)_NV_FLAGS),$(if $(PKG_interface_NV_FLAGS),$(PKG_interface_NV_FLAGS),$(if $(MODE_$(MODE)_NV_FLAGS),$(MODE_$(MODE)_NV_FLAGS),$(CMN_NV_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/interface/$(INC_DIR)) $(addprefix -I,$(PKG_interface_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_interface_INCS)) $(MODE_INCS) -c -o $@ $<

$(TMP_TOP)/$(MODE)/$(TMP_OBJ)/$(TMP_DYN)/$(PKG_DIR)/interface/$(SRC_DIR)/%.cu.o : $(PKG_DIR)/interface/$(SRC_DIR)/%.cu
	@if [ ! -d $(dir $@) ]; then \
		mkdir -p $(dir $@); \
	fi
	$(AT)rm -f $@
	@echo "    NVCC $(notdir $<) $(PRT_MODE) ($(NV_DYN_FLAGS))"
	$(AT)$(NVCC) $(NV_DYN_FLAGS) $(if $(PKG_interface_MODE_$(MODE)_NV_FLAGS),$(PKG_interface_MODE_$(MODE)_NV_FLAGS),$(if $(PKG_interface_NV_FLAGS),$(PKG_interface_NV_FLAGS),$(if $(MODE_$(MODE)_NV_FLAGS),$(MODE_$(MODE)_NV_FLAGS),$(CMN_NV_FLAGS)))) $(addprefix -I,$(PKG_DIR)) $(addprefix -I,$(PKG_DIR)/interface/$(INC_DIR)) $(addprefix -I,$(PKG_interface_MODE_$(MODE)_INCS)) $(addprefix -I,$(PKG_interface_INCS)) $(MODE_INCS) -c -o $@ $<
